...
export function WikiEditor() {
  ...
  const backlinkQueries = useQueries({
    queries: backlinkIds.length > 0 ? backlinkIds.slice(0, 10).map(backlinkId => ({
  ...
  const exportMutation = useMutation({
    mutationFn: async (format: 'md' | 'json') => {
      ...
      if (format === 'md') {
        const content = (data && (data.data?.content ?? data.content)) ?? (typeof data === 'string' ? data : JSON.stringify(data));
        return { blob: new Blob([content], { type: 'text/markdown;charset=utf-8' }), format };
      }
      // for json format, if data.data exists use it, otherwise use data directly
      const payload = data && (data.data ?? data);
      const jsonStr = typeof payload === 'string' ? payload : JSON.stringify(payload, null, 2);
      return { blob: new Blob([jsonStr], { type: 'application/json' }), format };
    },
