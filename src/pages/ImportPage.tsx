...
export function ImportPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: unknown) => toast.error(`Import failed: ${(error as Error).message}`),
  });
  ...
  const handleConfirmImport = () => {
    ...
    const hasOrigin = selectedPreviews.some(p => !!p.origin);
    const inferredSource = hasOrigin ? 'url' : (pasteContent.trim() ? 'paste' : 'upload');
    const payload: ImportPayload = {
      source: inferredSource,
      items: selectedPreviews.map(({ title, body, origin }) => ({ title, body, origin })),
    };
    if (!payload.items || payload.items.length === 0) {
      toast.warning('No items to import.');
      return;
    }
